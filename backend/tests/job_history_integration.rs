...
mod job_history_tests {
    use backend::data::job::{Job, JobTask};
    ...
    fn create_job_history_with_valid_data() {
        ...
        let entry = Job::new(tasks, 100.0, 2, "Project analysis".to_string());
    ...
    fn job_history_has_consistent_timestamps() {
        let tasks = vec![];
        let entry = Job::new(tasks, 100.0, 2, "Test".to_string());
    ...
    fn job_history_with_multiple_tasks() {
        ...
        let job_history = Job::new(tasks.clone(), 350.0, 5, "Multi-task project".to_string());
